...
def batch_detect(net, img_batch, device):
	...
	with torch.no_grad():
		...
		# img_batch.register_hook(lambda grad: print('non zero values ',torch.count_nonzero(grad)))
	
	# img_batch.register_hook(lambda grad: plt.imshow( np.transpose( grad[0].cpu().numpy(),(1,2,0) )  )        )
